...
void rand_array(float **vec, int size)
{
	int seed = 42;
	srand(seed);
...
void empty_array_gpu(float **dev_p, int size)
{
	cudaCHECK(cudaMalloc(dev_p, size * sizeof(float)));
}

bool validate(float *host_p, float *dev_p, int size)
{
	bool res = true;
	double eps = 1e-8;
